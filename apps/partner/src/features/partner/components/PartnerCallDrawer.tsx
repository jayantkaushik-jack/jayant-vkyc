...
import { formatAddress, formatDateLabel, formatTimeLabel } from '@vkyc/shared/lib/format';
...
/**
 * Partner-facing call drawer: exactly two tabs (View Details + Activity Log).
 * No video, no full KYC report — recordings and reports stay internal to the bank.
 * Staff identities are masked throughout.
 */

type DrawerTab = 'details' | 'activity';

interface PartnerCallDrawerProps {
  open: boolean;
  onClose: () => void;
  call: CallRecord;
  customer: Customer;
...
export function PartnerCallDrawer({ open, onClose, call, customer, agent }: PartnerCallDrawerProps) {
  ...
  return (
    <Modal open={open} onClose={onClose} title={`Call — ${customer.appId}`} size="lg">
      <div className="flex gap-2 border-b border-border mb-4 -mt-1">
        {(['details', 'activity'] as DrawerTab[]).map((t) => (
          <button
            key={t}
            type="button"
            onClick={() => setTab(t)}
          ...
          >
            {t === 'details' ? 'View Details' : 'Activity Log'}
          </button>
        ))}
      </div>
      ...
      {tab === 'details' ? (
        ...
        <ActivityTab call={call} customer={customer} agent={agent} />
      )}
    </Modal>
  );
}
  ...
  return (
    <div className="space-y-4 max-h-[68vh] overflow-y-auto pr-1">
      <div className="text-xs border border-border rounded-lg px-3 py-2 bg-gray-50 grid gap-1 md:grid-cols-3">
        <div><span className="text-text-muted">Application Type:</span> INDIVIDUAL</div>
...
function ActivityTab({ call, customer, agent }: { call: CallRecord; customer: Customer; agent: Agent }) {
  const maskedAgent = useMemo(
    () => ({ ...agent, name: maskStaffName(agent.id, 'agent') }),
    [agent],
  );
  const rows = useMemo(
    () => generateActivityLog(call, customer, maskedAgent).slice().sort((a, b) => a.timestamp.localeCompare(b.timestamp)),
    [call, customer, maskedAgent],
  );
