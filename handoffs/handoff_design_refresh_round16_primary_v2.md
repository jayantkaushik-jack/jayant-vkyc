...
### 1a. Correction — issues seen in the first implementation pass

The initial implementation of §1 had a few bugs; be explicit about these when re-implementing:

- **The Risk Profile column header must show the I/D/T/P/C initials** in a small row under the "Risk Profile" label (same sub-row as the dot row below it in each data row) — this was missing.
- **Merge "Rules Fired" and "Scenario" into a single column, "Rule Fired"**: primary line is the scenario/rule name (e.g. "SIM Circle Mismatch"), a smaller muted line underneath reads "{n} rules fired" (or "No rules fired"). Drop the separate Rules column entirely — this also removes the dead whitespace after Scenario/Status that resulted from having two separate columns.
- **The legend row ("Risk Profile: I Identity  D Digital Presence …") must be a sibling AFTER the scrollable row container, never a descendant of the `overflow-y: auto` element** — it should not scroll with the rows. Render it once, outside and below the scrolling `<div>`.
- Legend initials should be bold and dark (`font-weight:700`, `text-text` #1b1b1b), not the same muted gray as the label text next to them — they need to visually pop as the key.
- New column widths (7 columns, avatar/customer/band/score/risk-profile/rule-fired/status) — see updated prototype tab for exact `grid-template-columns`.
...
## 9. Latest visual pass — Customer Details & Decision Tree color/sizing fixes

- **Video Visible / Audible cards** (`CustomerDetailsStep.tsx`): were flat white-on-white — give each a tinted background + icon: Video Visible → light blue (`bg-accent-subtle`, blue camera icon in a filled circle), Audible → light green (`bg-success-subtle`, speaker icon in a filled circle). Selected "Yes" button is a solid fill in that card's color; "No" is an outlined button in the same hue — not identical to a plain gray dark/light choice.
- **Customer Details card**: section labels (Identity / Contact & Address / Account) each get a small colored dot + colored uppercase label (blue / green / amber respectively) instead of uniform gray, and the header avatar circle is filled with the accent color (white initials) instead of a flat gray circle — breaks up the monochrome look without adding clutter.
- **"Listen for applicant answer" button** (`AmberPanel.tsx` → `SpeechCapture`): was a full-width 14px-padding block, too large next to the two selects. Make it a compact pill (auto-width, ~10px vertical padding, `rounded-full`) sitting inline to the left of the language and bucket selects, not stacked full-width above them.
- **Rename** the "Simulate spoken answer" dropdown to **"Manually choose bucket"** (same select, same behavior — just clearer labeling for what it actually does).
- **Bucket/response list** ("Listening for one of these responses"): was plain white rows with no visual weight. Wrap the whole list in a very light tinted panel (`bg-bg` or a faint blue-gray, `rounded-lg p-3.5`), and give each response row a colored left border (3px) + a small circular radio indicator — vary the accent color per row is optional, but at minimum every row should read as a distinct, selectable option rather than plain text lines. This is the same "give it visual weight" treatment already used well in Quick Flags / End Session / Abort Call — extend that language here.
- Keep the **video panel's full-height/full-column layout unchanged** on both Customer Details and Decision Tree screens — no changes needed there, current sizing is correct.
